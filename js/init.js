...
function arlo_tm_scrollable(){
	...
	verMenu.css({height:"60%"});
	//verMenu.css({height:H-logoHeight-socialHeight});
	
console.log(H,logoHeight,socialHeight);

	scrollable.each(function(){
		var element		= jQuery(this);
		
		element.css({height: "60%"}).niceScroll({
			touchbehavior:false,
			cursorwidth:0,
			autohidemode:true,
...
function arlo_tm_animate_text(){
	...
	var animateSpan			= jQuery('.arlo_tm_animation_text_word');
		...
		animateSpan.typed({
			strings: ["Web Developer", "Qlik Consultant", "Power BI Consultant", "Quick Learner"],
			loop: true,
			startDelay: 1e3,
			backDelay: 2e3
		});
